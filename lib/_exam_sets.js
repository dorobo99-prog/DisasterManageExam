...
const SOURCE_SET_LABELS = {
  ...
  gemini_ch10: 'Gemini · 10장 실무 가이드북',
...
function parseQuestionId(questionId) {
  var match = String(questionId || '').match(/^(gemini|gpt)_(ch(?:10|[1-9]))_/);
...
function selectBalancedChapterQuestionIds(chapter, count, seenTexts) {
  var sourceSets = SOURCE_SETS_BY_CHAPTER[chapter] || [];
  if (!sourceSets.length || count <= 0) return [];

  var selected = [];
  var baseCount = Math.floor(count / sourceSets.length);
  var remainder = count % sourceSets.length;

  sourceSets.forEach(function(sourceSet, index) {
    selected = selected.concat(
      pickRandomUniqueQuestionIds(
        getQuestionIdPool(sourceSet),
  ...
  if (selected.length < count) {
    ...
    var remainingIds = getQuestionIdPool(chapter).filter(function(id) {
      return !selectedSet.has(id);
    });
    selected = selected.concat(
      pickRandomUniqueQuestionIds(remainingIds, count - selected.length, seenTexts)
    );
  }
...
function selectQuestions(setId) {
  ...
  if (setId === 'all') {
    var selectedIds = Object.keys(CHAPTERS).flatMap(function(chapter) {
      return selectBalancedChapterQuestionIds(
        chapter,
  ...
  return getQuestionsByIds(selectBalancedChapterQuestionIds(setId, 20, seenTexts));
}
