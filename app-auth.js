...
async function submitLogin(resetPin) {
  ...
  try {
    ...
    if (!data.ok) {
      if (data.code === "pin_mismatch") {
        ...
        const wantsReset = confirm(
          ...
          "확인을 누르면 이 닉네임의 기존 진행 기록과 학습 통계를 초기화하고, 방금 입력한 숫자 네 자리로 다시 시작합니다.\n\n" +
          "초기화 후에는 기존 점수, 완료 과목, 순위 정보가 새 기록 기준으로 다시 계산됩니다.\n\n" +
          "정말 새로 시작할까요?"
        );
        ...
        errEl.textContent =
          data.error || "처음 설정한 숫자 네 자리를 입력해야 로그인할 수 있습니다.";
      } else {
        nameInp.style.borderColor = "var(--red)";
        nameInp.focus();
    ...
    if (data.reset) {
      clearLocalUserState(loggedInName);
    }
    ...
    try {
      localStorage.setItem(LAST_USER_KEY, loggedInName);
    } catch (e) {}
